...
Une autre forme de validation (dans d'autres contextes) est de vérifier si les données avec lesquelles sont simulées. Plusieurs exemples de données sur Kaggle, comme les données sur le diabète sur Kaggle utilisées dans certains articles qui développent des modèles prédictifs, alors que les données, après analyse approfondie, montrent de clairs signes d'avoir été simulées [@Gibson:2026]. Seule une vérification minutieuse permet de détecter ce genre de problèmes.
...
g1 <- renfe |>
    dplyr::count(classe) |>
    dplyr::mutate(classe = forcats::fct_reorder(classe, n))  |>
ggplot(mapping = aes(y = classe, x = n)) + 
    geom_col() + 
    labs(subtitle = "classe", 
    x = "dénombrement", 
...
g2 <- renfe |>
    dplyr::count(type) |>
    dplyr::mutate(type = forcats::fct_reorder(type, n))  |>
ggplot(mapping = aes(y = type, x = n)) + 
    geom_col() + 
    labs(subtitle = "type de train", 
         x = "dénombrement", 
