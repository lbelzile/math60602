...
dbm_sub <- dbm |>
  dplyr::filter(test == 0) |>
  dplyr::select(!test) #|>
  # dplyr::rename(sexe = x1,
  #               age = x2,
  #               revenu = x3, 
  #               region = x4,
  #               couple = x5) |>
  # dplyr::mutate(achat = factor(yachat, 
  #               labels = c("non","oui"))) 
# skimr::skim(dbm_sub)
```
...
knitr::kable(table(dbm_sub$x3), 
             col.names = c("revenu", "décompte")),
knitr::kable(table(dbm_sub$x5), 
             col.names = c("couple", "décompte")),
knitr::kable(table(dbm_sub$x4), 
...
pos <- data.frame(methode = c("externe","validation croisée", "AIC", "BIC"),
