...
Dans le cas d'un modèle de régression linéaire multiple $Y = \beta_0 + \sum_{j=1}^p \beta_jX_j + \eps$ avec les erreurs $\eps \sim \mathsf{No}(0, \sigma^2)$ des termes indépendants et identiquement distributions, la log-vraisemblance du modèle pour un échantillon de taille $n$ est 
\begin{align*}
...
Ce qui lie les deux équations est le fait qu'on suppose que les variables sont binormales: les deux termes d'erreur sont de loi normale et sont corrélés, $\bs{\eps} \sim \mathsf{No}_2(\bs{0}_2, \bs{\Sigma})$. Les variables dépendantes observées sont :
\begin{align*}
