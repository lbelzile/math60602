...
\[ h(t) = - \frac{\d \log\{S(t)\}}{\d t}, \qquad \qquad S(t) = \exp \left\{ -\int_0^t h(u) \d u\right\}. \]
...
Testons l'hypothèse que la courbe de survie des clients masculins est la même que celle des clients féminins dans l'exmple des données d'abonnement. Ce test est effectué grâce à l'option `strata` de la procédure `lifetest`  à l'aide des commandes suivantes (voir le fichier `survival1_fonction_survie.sas`):
...
```{r fig5-e20, echo = FALSE, out.width='80%', fig.align = "center"}
...
- $\texttt{temps\_ch}$: temps au moment où un changement est survenu quant au nombre d'autres services. En l'absence de changement, l'observation est remplacée par une valeur manquante (`.`).
...
Les données pour cet exemple se trouvent dans le fichier `survival4.xls` et le programme dans le fichier `survival4_risques_competitifs.sas`. La seule nouveauté par rapport au fichier original est la variable $\texttt{censure}$ qui est maintenant codée ainsi
