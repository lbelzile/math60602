...
2. Décider comment seront mesurées les « distances » entre les sujets. 
3. Décider quel algorithme ou modèle sera utilisé.
4. Choisir les hyperparamètres de l'algorithme (nombre de regroupements, rayon, etc.), qui déterminent la qualité de la segmentation. 
5. Assigner les étiquettes aux observations et calculer un représentant-type de groupe. Interpréter les regroupements obtenus.
...
2) Examiner les caractéristiques démographiques de ces groupes.
...
1. il y aura beaucoup de graphes si le nombre de variables est grand et
2. on examine seulement les relations bivariées.
...
#| out-width: '100%' 
#| fig-align: "center"
#| fig-cap: "Nuage de point des paires d'observations des données `cluster`."
data(cluster, package = "hecmulti")
library(ggplot2)
library(patchwork)
scatter_fun = function(x, y) {
     ggplot(cluster, aes(x = .data[[x]], y = .data[[y]]) ) +
          geom_point() +
          theme_classic()
}
plot_list <- list()
...
Lorsque toutes les variables sont continues, la mesure de dissemblance la plus utilisée est la distance euclidienne entre sujets, soit
\begin{align*}
...
Le champ des applications des $K$-moyennes est parfois surprenant. Par exemple, [cet article de FiveThirtyEight propose une segmentation des électeurs démocrates new-yorkais](https://fivethirtyeight.com/features/the-5-political-boroughs-of-new-york-city/). Un autre exemple incongru est la compression d'images: la Figure @fig-decelles montre une image du bâtiment Decelles (coin supérieur gauche) et la reconstruction avec trois, quatre et 10 couleurs obtenues en appliquant l'algorithme des $K$-moyennes sur la matrice formée par les valeurs des canaux (rouge, vert, bleu) de l'image.
