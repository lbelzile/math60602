...
\begin{example}{}
\protect\hypertarget{exm:renfe-aed}{}{\label{exm:renfe-aed} \textbackslash iffalse (Analyse exploratoire des trains Renfe) \fi{} }La première étape consisterait à lire la description de la base de données. Le jeu de données \texttt{renfe} contient les variables suivantes
\end{example}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{proc corr data=multi.factor2;}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{proc factor data=multi.factor2 }
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{proc factor data=multi.factor2 method=principal}
...
\NormalTok{proc corr data=multi.factor2 alpha;}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{proc corr data=multi.factor2 polychoric out=poly\_corr;}
\NormalTok{var x1{-}x12;}
\NormalTok{run;}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{proc cluster data=temp method=ward outtree=temp1 nonorm rsquare ccc;}
\NormalTok{var x1{-}x6;}
...
{\centering \includegraphics[width=0.7\linewidth]{MATH60602_files/figure-latex/kmoyperfo-1} 
...
Le champ des applications des \(K\)-moyennes est parfois surprenant. Par exemple, \href{cet\%20article\%20de\%20FiveThirtyEight\%20propose\%20une\%20segmentation\%20des\%20électeurs\%20démocrates\%20new-yorkais}{https://fivethirtyeight.com/features/the-5-political-boroughs-of-new-york-city/}. Un autre exemple incongru est la compression d'images: la Figure \ref{fig:decelles} montre une image du bâtiment Decelles (coin supérieur gauche) et la reconstruction avec trois, quatre et 10 couleurs obtenues en appliquant l'algorithme des \(K\)-moyennes sur la matrice formée par les valeurs des canaux (rouge, vert, bleu) de l'image.
...
\begin{center}\includegraphics[width=0.7\linewidth]{MATH60602_files/figure-latex/02-plotEQMa-1} \end{center}
...
{\centering \includegraphics[width=0.7\linewidth]{MATH60602_files/figure-latex/validationcroiseeillust-1} 
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{proc glmselect data=trainymontant;}
\NormalTok{class x3(break) x4(break);}
\NormalTok{model ymontant=x1|x2|x3|x4|x5|x6|x7|x8|x9|x10@2 cx2 cx6 cx7 cx8 cx9 cx10  /   }
\NormalTok{selection=stepwise(select=aic choose=sbc) hier=none; }
\NormalTok{run;}
\end{Highlighting}
\end{Shaded}
...
{\centering \includegraphics[width=0.7\linewidth]{MATH60602_files/figure-latex/lassopenalty-1} 
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{proc glmselect data=trainymontant seed=57484765;}
\NormalTok{class x3(param=ref split) x4(param=ref split);}
\NormalTok{model ymontant=x1|x2|x3|x4|x5|x6|x7|x8|x9|x10@2 cx2 cx6 cx7 cx8 cx9 cx10  /   }
\NormalTok{selection=stepwise(select=aic choose=sbc); }
...
\NormalTok{                        xvariables=x1 x2 x31 x32 x41{-}x44 x5{-}x10,}
