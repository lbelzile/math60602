...
\begin{align*}
\ln \left\{\frac{\Pr(\texttt{catvote}_{1i}= \texttt{sporadiquement}\mid \texttt{age}_i)}{\Pr(\texttt{catvote}_{1i} = \texttt{rarement/jamais} \mid \texttt{age}_i)} \right\} &=
`r coefm1p11` + 
...
$`r expcoefm1`=\exp(`r coefm1`)$ pour chaque année de plus.

Ce résultat est logique, mais attention!
...
Soit $p_{ik}=\Pr(Y_i=k \mid \mathrm{X}_{i1}, \ldots, \mathrm{X}_{ip})$ ($k=1, \ldots, K$) la probabilité que $Y_{ik}$ prenne la valeur $k$. On dénote 
\begin{align*}
S_{ij}=\sum_{k=j}^K p_{ik}= \Pr(Y_{i} > j - 1 \mid \mathrm{X}_{i1}, \ldots, \mathrm{X}_{ip}), \qquad j=2, \ldots, K.
\end{align*}
La quantité $S_{ij}$ est la probabilité que $Y_i$ soit plus grand ou égal à $j$; $S_{i1}$ est égal à 1 et $S_{iK} = \Pr(Y_i=K \mid \mathrm{X}_{i1}, \ldots, \mathrm{X}_{ip})=p_{iK}$.
...
Il y a donc $K-1$ équations logistiques. Les paramètres quantifiant les effets des variables explicatives, $\beta_1, \ldots, \beta_p$ sont les mêmes pour chacune des log-cotes, mais il y a une ordonnée à l'origine différente par logarithme de rapport de cote. Par conséquent, pour modéliser une variable ordinale $Y$ ayant $K$ valeurs possibles et avec $p$ variables explicatives, le modèle cumulatif logistique utilise $p + K - 1$ paramètres. Le modèle logit multinomial, qui peut également être utilisé pour les données ordinales, utilise $(K-1) \cdot(p+1)$ paramètres. Le modèle logistique cumulatif à cotes proportionnelles est donc plus parcimonieux et, pour autant qu'il soit approprié, mènera à des estimations des paramètres plus précises qu'avec le modèle de régression logistique multinomiale. Les deux modèles sont identiques au modèle de régression logistique si la variable ordinale a uniquement deux modalités.
