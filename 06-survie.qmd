...
Une information pertinente de la sortie est nombre de données censurées: parmi les 500 observations, il y a 334 clients qui ont terminé leur abonnement et 166 qui sont censurées (le client est toujours abonné et le temps est donc une borne inférieure de la durée d'abonnement). Les données censurées contiennent moins d'information que les temps observés de défaillance puisqu'on sait uniquement la borne inférieure de la plage possible des valeurs pour le vrai temps de défaillance.
...
# Le test du log-rang calcule si les 
...
La fonction `survdiff` avec la formule retourne le résultat du test asymptotique du log-rang pour l'hypothèse d'égalité des fonctions de survie. La valeurs-$p$ du test est inférieure à $10^{-4}$: on rejette donc $\mathscr{H}_0$ pour conclure qu'il y a donc une différence significative entre les deux courbes de survie.
...
La sortie inclut notamment des tests de significativité globale basés sur la vraisemblance comparant le modèle sans variable explicative avec celui ajusté (rapport de vraisemblance, score et Wald). Ces trois statistiques ciblent la même hypothèse, aussi on peut ne s'attarder qu'au test du rapport de vraisemblance, qui est plus fiable et généralement plus puissant. Le tableau des coefficients donne les estimations $\widehat{\beta}$, le rapport de risque $\exp(\widehat{\beta})$ pour `sexe=1` versus `sexe=0` et le rapport de risque pour `sexe=0` versus `sexe=1`, soit $\exp(-\widehat{\beta})$. Les statistiques de test et la valeur-$p$ associée sont basées sur la statistique de Wald, soit $\widehat{\beta}/\mathsf{se}(\widehat{\beta})$, tandis que les intervalles de confiance à 95% pour le rapport de risque sont basés sur la vraisemblance profilée: un rapport de risque de 1 signifie que le risque n'est pas affecté par la variable explicative, et un intervalle de confiance 95% signifie que le test du rapport de vraisemblance correspondant pour l'hypothèse nulle $\beta=0$ ou $\exp(\beta)=1$ ne mène pas au rejet de l'hypothèse que la variable n'impacte pas le risque. Comme à l'accoutumée, on préfère les intervalles de confiance (qui correspondent au rapport de vraisemblance profilée) plutôt que la statistique de Wald pour l'inférence, puisque cette dernière dépend de la paramétrisation du modèle et l'approximation est mauvaise si la loi du paramètre est fortement asymmétrique.

```{r}
#| label: tbl-cox1 
#| eval: true
#| echo: false
#| tbl-cap: "Rapport de risques et intervalles de confiance basés sur la vraisemblance profilée à niveau 95%."
data(survie1, package = "hecmulti")
cox1 <- coxph(Surv(temps, censure) ~ sexe, 
              data = survie1,
              ties = "exact")
...
#| eval: true
#| echo: false
#| tbl-cap: "Rapport de risque et intervalles de confiance à niveau 95% basés sur la vraisemblance profilés pour le modèle de Cox de base avec toutes les variables explicatives."
cox3 <- coxph(Surv(temps, censure) ~ 
                age + sexe + region + service, 
              data = survie1, 
              ties = "exact")
...
Finalement, la variable $\texttt{region}$ n'est pas globalement significative (statistique du rapport de vraisemblance de `r round(as.numeric(tab2[3,2]),2)` avec une valeur-$p$ de `r tab2[3,4]`)) basée sur une loi $\chi^2_4$.
...
Le modèle de Cox fait l'hypothèse que la fonction de risque de base est identique peut importe la valeur des variables catégorielles. Si ce n'est pas le cas, alors les résultats du modèle ne sont pas nécessairement fiables.
...
#| eval: true
#| echo: false
#| tbl-cap: "Postulat de risques proportionnels: test du score de Grampsch et Therneau (1994) pour les coefficients constants dans le temps et valeur-p asymptotique basée sur la loi null khi-deux." 
...
Dans la @fig-coxph-hypothese, on voit que le coefficient pour service augmente au fil du temps pour tous les groupes. On pourrait capturer cette interaction ou stratifier pour calculer le risque selon le nombre de services, au risque d'avoir trop peu d'observations pour estimer de manière fiable le risque de base. C'est explicable par le fait que les personnes avec plus de services tendent à avoir plus une survie plus longue.
...
Le plus difficile est de créer correctement le fichier de données pour effectuer ce genre d'analyse. Pour chaque personne, on peut identifier plusieurs moments où l'une ou l'autres des valeurs des variables explicatives change. Supposons qu'on a observé un événement au temps $t_1$, et que la valeur de la covariable change à $t_0$, où $0 < t_0 < t_1$. On peut envisager cette observation donne deux contributions: une pour la trajectoire sur l'intervalle $(0, t_0]$ et l'autre, après la modification, sur l'intervalle $(t_0, t_1]$. Puisque la valeur est observée passée le premier intervalle, on a besoin de l'information sur toute la fenêtre (les deux bornes) et le type d'événement pour la première fenêtre sera de la censure à droite.
...
L'interprétation se fait comme précédemment; on a omis l'option `ties = "exact"` parce que cette option est trop gourmande en calcul. Puisque c'est la valeur d'une variable qui varie dans le temps et non pas son effet, on a l'interprétation usuelle. Par exemple, le rapport de risque de quitter pour un client qui a un autre service est `r round(exp(coef(cox4))['service1'], 3)` fois celui d'un client qui n'a aucun autre service (référence). Le fait d'avoir deux ou trois services diminue encore plus le risque de quitter (ratios de risque de `r round(exp(coef(cox4))['service2'], 3)` et `r round(exp(coef(cox4))['service3'], 3)`, respectivement). 
...
#| echo: false
#| eval: true
#| tbl-cap: "Rapport de risque et intervalles de confiance à niveau 95% basés sur la vraisemblance profilés pour le modèle à risques compétitifs (probabilité de quitter pour compétiteur A)."
data(survie4, package = "hecmulti")
cox5 <- coxph(Surv(time = temps, 
                   event = censure == 2,
                   type = "right") ~ 
...
Ajustons le modèle pour l'abonnement cellulaire en ajoutant une interaction entre l'âge et le temps. 
On spécifie avec l'option `tt()` la variable qui change dans le temps et on spécifie par la suite la nature de l'interaction temporelle.
...
#| echo: false
#| eval: true
#| tbl-cap: "Rapport de risque et intervalles de confiance à niveau 95% basés sur la vraisemblance profilés pour le modèle à risques non proportionnels (interaction linéaire entre temps et âge)."
cox6 <- survival::coxph(
    Surv(temps, censure) ~ 
     age + tt(age) + sexe + region + service, 
     data = survie1, 
...
L'interaction entre l'âge et le temps est spécifiée en incluant `tt`: la variable produit `age * t` est créée à l'intérieur de l'appel à la fonction `coxph`, et qui est égale au produit entre l'$\texttt{age}$ et $\texttt{t}$. On remarque que le terme d'interaction est tout juste non significatif selon le test de Wald (valeur-$p$ de `r format.pval(eps = 1e-4, summary(cox5)$coefficients[2,5])`).^[Pour des raisons techniques hors du niveau du cours, le test du rapport de vraisemblance pour l'effet de l'interaction est probablement illogique ici.] 

L'interaction employée ici n'est pas la seule fonction du temps qu'on pourrait spécifier, ni la plug logique. Par exemple, on pourrait spécifier un effet de l'âge par pallier selon une catégorie d'âge, ou un spline pour un effet plus flexible.
...
cox7 <- coxph(Surv(temps, 1-censure) ~ 
                age + sexe + strata(region) + service, 
              data = survie1,
              ties = "exact")
# Décompte par région
with(survie1, table(region))
# Coefficients
summary(cox7)
```
...
#| tbl-cap: "Rapport de risques et intervalles de confiances à niveau 95% basés sur la vraisemblance profilée pour un modèle de Cox stratifié par région."
#| eval: true
#| echo: false
cox7 <- coxph(Surv(temps, 1-censure) ~ 
                age + sexe + service + strata(region), 
              data = survie1,
              ties = "exact")
...
On voit à la lecture de la sortie dans le @tbl-cox-stratif qu'il n'y a pas de paramètres pour la variable `region`. Les paramètres des autres variables s'interprètent comme d'habitude.
