...
{\centering \includegraphics[width=0.9\linewidth]{MATH60602_files/figure-latex/acprotation-1} 
...
{\centering \animategraphics[width=0.8\linewidth,controls,loop]{1}{MATH60602_files/figure-latex/wardAnimation-}{1}{1}

}

\caption[Regroupements hiérarchiques avec la méthode de Ward (solution à trois groupes)]{Regroupements hiérarchiques avec la méthode de Ward (solution à trois groupes)}\label{fig:wardAnimation}
\end{figure}
...
\begin{align*}
\mathsf{AIC} &=-2 \ell(\widehat{\boldsymbol{\beta}}, \widehat{\sigma}^2) +2p = n \ln (\mathsf{EMQ}) + 2p + \text{constante},
\end{align*}
tandis que le critère d'information bayésien de Schwartz, \(\mathsf{BIC}\), est défini par
\begin{align*}
\mathsf{BIC} &=-2 \ell(\widehat{\boldsymbol{\beta}}, \widehat{\sigma}^2) + p\ln(n)=n \ln (\mathsf{EMQ}) + p\ln(n) + \text{constante}.
\end{align*}
...
{\centering \includegraphics[width=0.7\linewidth]{MATH60602_files/figure-latex/demandecredit-1} 
...
Dans le cas d'un modèle de régression linéaire multiple \(Y = \beta_0 + \sum_{j=1}^p \beta_jX_j + \varepsilon\) avec les erreurs \(\varepsilon\sim \mathsf{No}(0, \sigma^2)\) des termes indépendants et identiquement distributions, la log-vraisemblance du modèle pour un échantillon de taille \(n\) est
\begin{align*}
...
\item
  le \textbf{taux de vrais positifs}, \({\mathsf P}\left(Y=1 \mid \widehat{Y}=1\right)\), ou \(\mathsf{VP}/(\mathsf{VP}+\mathsf{FP})\);
\item
  le \textbf{taux de vrais négatifs}, \({\mathsf P}\left(Y=0 \mid \widehat{Y}=0\right)\), ou \(\mathsf{VN}/(\mathsf{VN}+\mathsf{FN})\).
\end{itemize}
...
\begin{Highlighting}[]
\NormalTok{proc logistic data=train;}
\NormalTok{class x3 x4 / ref=glm;}
\NormalTok{model yachat(ref=\textquotesingle{}0\textquotesingle{}) = x1{-}x10;}
\NormalTok{output out=pred predprobs=crossvalidate;}
\NormalTok{run;}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{\%manycut\_cvlogisticclass(}
\NormalTok{  yvar=yachat, xvar=x1{-}x10, xvarclass=x3{-}x4,}
...
Soit \(p_{ik}={\mathsf P}\left(Y_i=k \mid X_{i1}, \ldots, X_{ip}\right)\) (\(k=0, 1, \ldots, K\)) la probabilité que \(Y_{ik}\) prenne la valeur \(k\). On dénote
\[S_{ij}=\sum_{k=j}^K p_{ik}= {\mathsf P}\left(Y_{i} > j - 1 \mid X_{i1}, \ldots, X_{ip}\right), \qquad j=1, \ldots, K.
\]
