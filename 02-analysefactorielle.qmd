...
::: {#fig-datasaurus}
![](figures/DataSaurusDozen.gif)

 Datasaurus (Alberto Cairo): une douzaine de jeux de données qui ont les mêmes statistiques descriptives (à deux décimales près) et une faible corrélation, mais qui sont visuellement distincts.
:::
...
\end{align*}
une combinaison des vecteurs propres $\boldsymbol{q}_1, \ldots, \boldsymbol{\gamma}_k \in \mathbb{R}^p$ non corrélés.
...
options(knitr.kable.NA = '')
knitr::kable(100*chargements, digits = 0, row.names = TRUE) |>
  kableExtra::kable_styling(full_width = TRUE)
```
...
emv_crit <- hecmulti::adjustement_factanal(
    covmat = cov(factor),
    factors = 1:5,
    n.obs = nrow(factor))
colnames(emv_crit)[4] <- "valeur-p"
knitr::kable(emv_crit, digits = c(2,2,2,0,0)) |>
  kableExtra::kable_styling(full_width = TRUE)
```
...
# Cohérence interne (alpha de Cronbach)
alph <- c(
  hecmulti::alpha_Cronbach(factor[,c("x4","x8","x11")]),
  hecmulti::alpha_Cronbach(factor[,c("x3","x6","x9","x12")]),
  hecmulti::alpha_Cronbach(factor[,c("x2","x7","x10")]),
  hecmulti::alpha_Cronbach(factor[,c("x1","x5")]))
