...
knitr::kable(a, caption = "Nombre de billets au tarif Flexible selon le prix de vente.", longtable = FALSE, booktabs = TRUE)
```
...
<!--chapter:end:01-analyseexploratoire.Rmd-->
...
<!--chapter:end:02-analysefactorielle.Rmd-->
...
```{r fig4-e1, echo = FALSE, out.width='80%', fig.align = "center"}
...
```{r 04-pairplot, echo = FALSE, eval = TRUE, fig.width = 7, fig.height=7, out.width='100%', fig.align = "center"} 
...
2) On assigne dans l'ordre les observations au groupe le plus proche en utilisant la distance euclidienne par rapport au germe. Soit on assigne toutes les observations avant de mettre à jour les germes, soit on met à jour les germes après chaque assignation d'un sujet. Le nouveau germe d'un groupe est la moyenne des observations faisant partie du groupe.
...
x4 <- mlbench::mlbench.spirals(200,1,0.025)$x
...
<!--chapter:end:03-regroupements.Rmd-->
...
<!--chapter:end:04-selectionmodeles.Rmd-->
...
Dans le cas d'un modèle de régression linéaire multiple $Y = \beta_0 + \sum_{j=1}^p \beta_jX_j + \eps$ avec les erreurs $\eps \sim \mathcal{N}(0, \sigma^2)$ des termes indépendants et identiquement distributions, la log-vraisemblance du modèle pour un échantillon de taille $n$ est 
\begin{align*}
...
run;
```
La syntaxe `y(ref='0')` sert à spécifier la catégorie de référence, zéro, de la variable réponse $Y$: le modèle décrit donc $\P{y=1 \mid X_5}$.
...
Les variables incluses dans la commande `class` sont modélisées à l'aide d'un ensemble de variables indicatrices. Prenons l'exemple de la variable $X_1$: la modalité de référence est (\texttt{5}), soit agriculture est spécifiée dans le tableau `Informations sur les niveaux de classe`.
...
Prenons comme exemple le test de la significativité de $X_6$, qui est modélisée à l'aide deux variables binaires $\I{X_6=1}$ et $\I{X_6=2}$ et dont les paramètres correspondants sont $\beta_{6_{\texttt{1}}}$ et $\beta_{6_{\texttt{2}}}$. Nous avons déjà étudié la sortie pour le test de Wald de significativité globale de $X_6$, soit le test de l'hypothèse $\Hy_0: \beta_{6_{\texttt{1}}}=\beta_{6_{\texttt{2}}}=0$ versus l'alternative qu'au moins un de ces deux paramètres est différent de zéro. La statistique de test (de Wald) est $42.93$ et la valeur-$p$ est moins de $10^{-4}$. Pour effectuer le test du rapport de vraisemblance, il suffit de retirer la variable $X_6$ et de réajuster le modèle à nouveau avec toutes les autres variables; cette manipulation est effectuée dans `logit1_intro.sas`. On obtient donc 
...
- le **taux de faux positifs**, $\P{Y=0 \mid \widehat{Y}=1}$, ou $\mathsf{FP}/(\mathsf{VP}+\mathsf{FP})$;
- le **taux de faux négatifs**, $\P{Y=1 \mid \widehat{Y}=0}$, ou $\mathsf{FN}/(\mathsf{VN}+\mathsf{FN})$.
...
```{r fig3-e17, fig.cap="Taux de classement en fonction du lift", echo = FALSE, out.width='80%', fig.align = "center"}
...
### Recherche exhaustive de tous les sous-modèles
...
Une stratégie similaire est possible avec la procédure `logistic` sauf qu'au lieu de trouver une suite de meilleurs modèles selon le $R^2$, un autre critère est retenu. Il s'agit de la statistique du score (ou test des multiplicateurs de Lagrange) pour tester si l'ajout d'une variable est utile ou pas.
...
Faisons d'abord une recherche séquentielle classique avec les 104 variables, en prenant 0.05 comme critère d'entrée et de sortie. Les options **SAS** correspondantes sont
`selection=stepwise`, avec comme départ le nombre de variables `start=104` et les niveaux `sle=.05` et `sls=.05`;
```{sas 03-stepwise, eval = FALSE, echo = FALSE}
proc logistic data=train;
...
```{r fig3_e22, echo = FALSE, out.width='80%', fig.align = "center"}
knitr::include_graphics("figures/03-logistic-e22.png")
```


Il y a eu 17 étapes dans la recherche séquentielle débutant avec le modèle qui ne contient que l'ordonnée à l'origine. La variable `i_x7_x8` est la première à être incluse, suivie de `x2` et ainsi de suite. Le modèle final contient 13 variables.
En faisant une recherche séquentielle avec 0.5 comme comme critère d'entrée et de sortie, nous obtenons un sous-ensemble de 47 variables. On peut ensuite faire une recherche exhaustive de tous les sous-modèles avec ces 47 variables en utilisant la macro; le fichier `logit5_selection_variables.sas` contient le code commenté.
...
Ce qui lie les deux équations est le fait qu'on suppose que les variables sont binormales: les deux termes d'erreur sont de loi normale et sont corrélés, $\bs{\eps} \sim \mathcal{N}_2(\bs{0}_2, \bs{\Sigma})$. Les variables dépendantes observées sont :
\begin{align*}
Y_{1}^{\star} &= \beta_{01} + \beta_{11} X_{11} + \cdots + \beta_{1p}X_{p1} + \eps_{1}\\
Y_{2}^{\star} &= \beta_{02} + \beta_{12} X_{12} + \cdots + \beta_{1p}X_{q2} + \eps_{2}
\end{align*}
...
\begin{align*}
\ln\left(\frac{p_i}{1-p_i}\right) = \beta_0 + \beta_1 X_{i}, \qquad p_i = \P{Y_i=1 \mid X_i} = \expit(\beta_0 + \beta_1X_i).
\end{align*}
Dans ce modèle logistique, $\ln(p_i)-\ln(1-p_i) = \ln\{\P{Y_i=1 \mid X_i}\} -  \ln\{\P{Y_i=0 \mid X_i}\}$ peut être vu comme étant le logit de la catégorie 1 en utilisant 0 comme catégorie de référence. 
...
\end{align*}
où $p_{ik} = \P{Y_i=k \mid X_i}$ $(k=0, \ldots, K)$. Comme en régression logistique, on peut facilement exprimer ce modèle en termes des différentes probabilités,
\begin{align*}
 p_{i0} &= \P{Y_i=0 \mid X_i} = \frac{1}{1+ \sum_{j=1}^K\exp(\beta_{0j}+\beta_{1j}X_i)}\\
 p_{ik} &= \P{Y_i=k \mid X_i} = \frac{\exp(\beta_{0k}+\beta_{1k}X_i)}{1+ \sum_{j=1}^K\exp(\beta_{0k}+\beta_{1k}X_i)}, \qquad k =1, \ldots, K.
\end{align*}
...
\begin{align*}
\ln \left\{\frac{\P{Y_{1i}=1 \mid X_i}}{\P{Y_{1i}=0 \mid X_i}} \right\} = -4.10 + 0.13X_i, \qquad \qquad \ln \left\{\frac{\P{Y_{1i}=2 \mid X_i}}{\P{Y_{1i}=0 \mid X_i}} \right\} = -7.98+0.22X_i. 
\end{align*}
...
Soit $p_{ik}=\P{Y_i \mid X_{i1}, \ldots, X_{ip}}$ ($k=0, 1, \ldots, K$) la probabilité que $Y_{ik}$ prenne la valeur $k$. On dénote la fonction de survie
\[S_{ij}=\sum_{k=j}^K p_{ik}= \P{Y_{i} \geq j\mid X_{i1}, \ldots, X_{ip}}, \qquad j=1, \ldots, K.
\]
La quantité $S_{ij}$ est la probabilité que $Y_i$ soit plus grand ou égal à $j$; $S_{i0}$ est égal à 1 et $S_{ik} = \P{Y_i=K \mid X_{i1}, \ldots, X_{ip}}=p_{iK}$.
...
\begin{align*}
S_{ik} = \P{Y_i \geq k \mid X_{i1}, \ldots, X_{ip}} = \expit(\beta_{0k} + \beta_1 X_{i1} + \cdots + \beta_p X_{ip}), \qquad j =1, \ldots, K.
\end{align*}
En utilisant ces expressions, on peut obtenir la probabilité de chaque catégorie,
\begin{align*}
\P{Y_i = k \mid X_{i1}, \ldots, X_{ip}} =\P{Y_i \geq k \mid X_{i1}, \ldots, X_{ip}} -\P{Y_i \geq k+1 \mid X_{i1}, \ldots, X_{ip}} = S_{k} - S_{k+1}.
\end{align*}
...
<!--chapter:end:05-reglogistique.Rmd-->
...
Le modèle à risques proportionnels de Cox (_proportional hazard model_) est l'un des modèles les plus utilisés pour analyser des données de survie. 
...
<!--chapter:end:07-donneesmanquantes.Rmd-->
