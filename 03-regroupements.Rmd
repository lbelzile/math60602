...
```{r 04-pairplot, echo = FALSE, eval = TRUE, fig.width = 9, fig.height=16, out.width='90%', fig.align = "center"} 
url <- "https://lbelzile.bitbucket.io/MATH60602/cluster1.sas7bdat"
cluster1 <- haven::read_sas(url)
library(ggplot2)
library(patchwork)
scatter_fun = function(x, y) {
     ggplot(cluster1, aes(x = .data[[x]], y = .data[[y]]) ) +
          geom_point() +
          theme_classic()
}
plot_list <- list()
...
for(i in 1:5){
  for(j in ((i+1):6)){
    ind <- ind + 1L
    plot_list[[ind]] <- scatter_fun(paste0("x",i), paste0("x",j))
  }
}
cowplot::plot_grid(plotlist = plot_list,nrow = 5, ncol = 3)
# pairs(cluster1[,1:6], 
...
La figure \@ref(fig:acprotation) démontre cette décomposition sur des données simulées. La variance des données dans le premier panneau est `r round(var(data)[1,1], 2)` pour l'axe des abscisse et `r round(var(data)[2,2], 2)` pour l'axe des ordonnées avec une corrélation de `r round(cor(data)[1,2],2)`, à comparer avec des variances de `r round(pc$sdev[1]^2,2)` et `r round(pc$sdev[2]^2, 2)` et une corrélation nulle entre les deux composantes principales.
...
```{r 04-acp, eval = TRUE, echo = FALSE, out.width='90%', fig.align = "center", fig.cap = "Projection des observations sur les composantes principales avec les regroupements finaux créés à la fin du chapitre."}
library(ggplot2)
url <- "https://lbelzile.bitbucket.io/MATH60602/cluster1.sas7bdat"
cluster1 <- haven::read_sas(url)
par(mar = c(4,4,1,1), mfrow = c(1,2))
clusters <- kmeans(x = cluster1[,1:6], centers = 3)$cluster
cp <- princomp(cluster1[,1:6])$scores[,1:2]
